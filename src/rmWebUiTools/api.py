...
class RmFile:
    ...
    def __init__(self, metadata, parent=None):
        ...
        self.name = (
            ...
            if "VissibleName" in metadata
            else metadata["VisibleName"]
        )  # Typo from reMarkable which will probably get fixed in next patch
        self.id = metadata["ID"]
        ...
        self.modifiedTimestamp = datetime.strptime(
            metadata["ModifiedClient"], "%Y-%m-%dT%H:%M:%S.%fZ"
        ).timestamp()
    ...
    def exportPdf(self, targetPath):
        ...
        response = requests.get(
            RM_WEB_UI_URL + "/download/" + self.id + "/placeholder", stream=True
        )
...
def iterateAll(filesOrRmFile):
    ...
    if isinstance(filesOrRmFile, RmFile):
        ...
        # Unknown type found!
        raise ValueError(
            '"api.iterateAll(filesOrRmFile)" only accepts (nested) iterables and instances of class api.RmFile !'
        )
...
def fetchFileStructure(parentRmFile=None):
    ...
    if parentRmFile and not parentRmFile.isFolder:
        raise ValueError(
            '"api.fetchFileStructure(parentRmFile)": parentRmFile must be None or of type folder!'
        )
    ...
    # Get metadata:
    directoryMetadataUrl = (
        RM_WEB_UI_URL + "/documents/" + (parentRmFile.id if parentRmFile else "")
    )
    ...
    if not response.ok:
        raise RuntimeError(
            "Url %s responsed with status code %d"
            % (directoryMetadataUrl, response.status_code)
        )
